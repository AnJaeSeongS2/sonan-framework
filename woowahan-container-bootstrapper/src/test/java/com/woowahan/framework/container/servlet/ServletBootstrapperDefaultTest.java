...
class ServletBootstrapperDefaultTest {
    ...
    void selfInitialize() throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, NoSuchFieldException {
        ...
        Util.invokeMethodAnyway(bootstrapper, "initBeanDefinitionHolder", new Class<?>[]{URLClassLoader.class, String.class}, (URLClassLoader) Thread.currentThread().getContextClassLoader(), "com.woowahan.framework.container.servlet.beanInOfBasePackage");
        Util.setFieldAnyway(bootstrapper, "rootAppCtx", rootApplicationContext);

        // test target
        Util.invokeMethodAnyway(bootstrapper, "selfInitialize", new Class<?>[]{ServletContext.class}, servletContext);
        GenericApplicationContext<ServletContext> rootAppCtx = (GenericApplicationContext<ServletContext>) Util.getFieldAnyway(bootstrapper, "rootAppCtx");
